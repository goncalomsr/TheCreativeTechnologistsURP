    ...
    private void Draw()
    {
        if (Physics.Raycast(origin:_tip.position, direction:transform.up, out _touch, _tipHeight))
        {
            if (_touch.transform.CompareTag("Whiteboard"))
            {
                ...
                {
                    _whiteboard.texture.SetPixels(x, y, blockWidth: _penSize, blockHeight: _penSize, _color);
                    
                    for (float f = 0.01f; f < 1.00f; f += 0.01f)
                    {
                        var lerpX = (int)Mathf.Lerp(a: _lastTouchPos.x, b: x, t: f);
                        var lerpY = (int)Mathf.Lerp(a: _lastTouchPos.y, b: y, t: f);
                        _whiteboard.texture.SetPixels(lerpX, lerpY,blockWidth: _penSize,blockHeight: _penSize, _color);
                    }
